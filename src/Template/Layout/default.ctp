...
<meta name="description" content="<? if ($wpis) { ?> <?=$wpis['firstname']?> pisze do prezydenta Andrzeja Dudy: <?=$wpis['body']?><? } else { $ilesend=$ilemails/100*100; ?>Już ponad <?=$ilesend?> osób wysłało e-maila do Prezydenta ze swoją wizją reformy sądownictwa! Tu możesz je przeczytać.
...
<meta property="og:description" content="? if ($wpis) { ?> <?=$wpis['firstname']?> pisze do prezydenta Andrzeja Dudy: <?=$wpis['body']?><? } else { $ilesend=$ilemails/100*100; ?>Już ponad <?=$ilesend?> osób wysłało e-maila do Prezydenta ze swoją wizją reformy sądownictwa! Tu możesz je przeczytać.
 A Ty, jakich sądów chcesz? Kliknij w link, aby przejść na stronę i wysłać wiadomość do Andrzeja Dudy!<? }; ?>" />
