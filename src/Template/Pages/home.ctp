...
<script>
 WebFont.load({
    custom: {
      ...
      active: function() {$(window).bind("load", function() {
	<? if ($wpsdis) { ?>
	$("#wpisModal").modal('show');
...
<style>
	...
	@font-face {
  font-family: 'din';
  src: url('../font/FFDINWebProCondBold.ttf'); /* IE9 Compat Modes */
  src: 
...
<div class="switches hidden-xs-up">
  ...
  <option>Wszystkie listy</option>
   <option <? if ($_GET['streams']==6) { echo 'selected'; }; ?> value="30">30</option>
  <option <? if ($_GET['streams']==12) { echo 'selected'; }; ?> value="60">60</option>
  <option <? if ($_GET['streams']==18) { echo 'selected'; }; ?> value="90">90</option>
  <option <? if ($_GET['streams']==24) { echo 'selected'; }; ?> value="120">120</option>
</select>
 
		 
		 
	 </div>
	 		<span class="hidden-xs-up">Losowo</span> 

  <div class="float-right ml-auto" id="navbarSupportedContent">
<ul class="navbar-nav ml-auto mr-1 hidden-xs-up float-right">
...
<li><a href="https://www.akcjademokracja.pl/kampanie/">Kampanie</a></li>
<li><a href="https://www.akcjademokracja.pl/o-nas/">O nas</a></li>
<li><a href="https://www.akcjademokracja.pl/wspolpraca/">Współpraca</a></li>
<li><a href="https://dzialaj.akcjademokracja.pl/campaigns/dorzuc-sie">Dorzuć się </a></li>
</ul>
</div></div>
<div class="container-fluid" style="padding:10px 20px; background:#f9f9f9; margin-top:75px; margin:60px 15px 0px 15px">
	<div class="row justify-content-around">
		<div class="col-sm-6 text-center text-sm-left">
			Piszemy do Prezydenta Andrzeja Dudy w obronie niezależności sądów
		</div>
		<div class="col-sm-6 text-sm-right text-center">
			Wysłano już <span class="text-danger"><?=$ilemails?></span> listów
...
js.src = "http://connect.facebook.net/pl_PL/all.js";
 ...
 <div class="container" style="padding:20px; background:#efefef; margin-top:50px" style="">
	 ...
	 <h5 class="text-300 line-height-30"><?=$wpis['body']?></h5>
	 		<h6 class="text-muted"><?=$wpis['firstname']?> <? if ($wpis['miejscowosc']) { echo '- '.$wpis['miejscowosc'].''; }; ?> - <?=DATE('d.m.Y', strtotime(''.$wpis['created_at'].''))?></h6>
<br/>
 ...
 <?   foreach ($mails as $stream) { if (count($mails)<9) { $lg=6; } else { $lg=4; } if ($_GET['small']==1) { $lg=4; }  ?>
<div class="col-lg-<?=$lg?> col-md-6 col-12 grid-item" style="display:block; padding:15px; margin-bottom: 10px;">
	<div class="card" style="padding: 20px; position: relative"><? if ($user) { ?> <div class="accept-this <? if ($stream['active']==1) { echo 'bg-success active'; };  if ($stream['active']==0) { echo 'bg-info'; };  if ($stream['active']==2) { echo 'bg-danger'; }; ?>" data-id="<?=$stream['id']?>"><? if ($stream['active']==2) { echo '<span class="flaticon-cancel"></span>'; } else { ?><span class="flaticon-checked"></span><? }; ?></div><? }; ?>
		...
		<h6 class="text-muted"><?=$stream['firstname']?> <? if ($stream['miejscowosc']) { echo '- '.$stream['miejscowosc'].''; }; ?> - <?=DATE('d.m.Y', strtotime(''.$stream['created_at'].''))?></h6>
...
<script> WebFont.load({
	...
	if ($(this).hasClass('active')) { 
		$(this).removeClass('active bg-success').addClass('bg-danger');
		$(this).find('span').removeClass('flaticon-checked').addClass('flaticon-cancel');
		co=2;
	} else { 
		$(this).removeClass('bg-danger bg-info').addClass('bg-success active');
		$(this).find('span').removeClass('flaticon-cancel').addClass('flaticon-checked');
		co=1;
	}
 ...
 $.ajax({
 				type: "post",
 				url: "/users/mailupdate", 
				data: {id:id, co:co, funkcja:'active',sid:'<?=$sid?>',xdbf:'<?=$xdbf?>'}, 
				dataType: 'json',
				beforeSend: function() {
 				},
